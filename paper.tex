...
To formalize (and prove) this fact,
we must first decide how to encode Turing machines in strings
(because all a Turing machine can manipulate are strings).
...
We will use the concept of \emph{acceptable Gödel numbering}
(\cite[p.~41]{Rogers1987},~\cite[p.~324]{Blum1967})
to impose reasonableness criteria
to our definitions.
...
\begin{definition}
    ...
    that associates each string (or program) $w \in \{0, 1\}^*$
...
By varying the first input of~$U$,
we can obtain any partial recursive function in a controlled manner.

And Condition~\ref{smn-theorem} is known as the $S_{mn}$ theorem,
or the iteration theorem.
