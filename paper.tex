...
we want not to fall in the realm of undecidable problems,
but neither we want to ``forget'' of some computable function.
Therefore,
the new definition must be \emph{computationally equivalent}
to the deterministic machines.

In the ``non-deterministic'' part,
we want to preserve the fact that
converting to deterministic machines seems
\footnote{
...
to require an exponential slowdown.
Therefore,
...
\footnotetext{
    ...
    is the function $1_A$ defined by
    \begin{equation*}
        1_A(x) = \begin{cases}
            1, & \text{if $x \in A$}. \\
            0, & \text{if $x \notin A$}.
...
formalize our requirements for the ``computation'' part
