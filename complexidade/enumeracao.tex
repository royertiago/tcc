...
De posse dos teoremas da máquina universal e $s-m-n$,
...
\begin{equation*}
    g(x, y) = \phi_x(y).
\end{equation*}
Similarmente, por ``satisfazer ao teorema $s-m-n$'' queremos dizer que
...
\begin{example}
    O mapeamento $x \mapsto f_x$ é uma enumeração de Gödel aceitável.
\end{example}
...
(possuir uma função universal e satisfazer ao teorema $s-n-m$)
