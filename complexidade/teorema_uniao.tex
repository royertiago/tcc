...
\begin{proof}
    ...
    então $\phi_w \in \mathcal C_\Phi(f_k)$ para algum $k$,
    mas também que $\phi_w \in \mathcal C_\Phi(f_i)$
    ...
    da forma $\phi_{w_i} \in \mathcal C_\Phi(f_j)$,
    ...
    Caso $\phi_{w_i}$ esteja em algum $\mathcal C_\Phi(f_k)$,
    ...
    Então, se $\phi_{w_i} \in \mathcal C_\Phi(f_k)$,
    ...
    terá, para todo $k$,
    $\Phi(w_j, x) > f_k(|x|)$ para infinitos $x$.
...
Se $\phi_{w_t} \in \mathcal C_\Phi(f_i)$,
