...
\begin{frame}
    ...
    \begin{tikzpicture}[auto,every node/.style={font=\footnotesize}]
        ...
        \draw (r2) to [out=170, in=10] node {$\DSPACE(n^c)$} (l2);
        \draw (r3) to [out=170, in=10] node {$\DSPACE(n^{c+\varepsilon})$} (l3);
        \draw (r4) to [out=170, in=10] node {$\DSPACE(n^{c+2\varepsilon})$}
                      node [swap] {$\vdots$} (l4);
    \end{tikzpicture}
\end{frame}
